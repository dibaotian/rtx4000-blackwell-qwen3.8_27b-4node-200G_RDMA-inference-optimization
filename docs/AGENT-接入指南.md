...
```
Base URL:  http://<NODE0_PUBLIC_IP>:8000/v1   ← node0 对外业务网 IP
...
> ⚠️ **IP 说明（重要）**：node0 有多个 IP，用错连不上：
> - **`<NODE0_PUBLIC_IP>`（业务/管理网，<对外网卡>）← agent 从外部连这个**
> - `<NODE0_IP>`（内网 RDMA/Ray，ens7）：仅集群内部用，agent 不要用
> - vLLM 监听 `0.0.0.0:8000`，所以任意网卡的 IP:8000 都能访问；对外一律用 `<NODE0_PUBLIC_IP>`。

### 最小调用示例
```bash
curl http://<NODE0_PUBLIC_IP>:8000/v1/chat/completions \
